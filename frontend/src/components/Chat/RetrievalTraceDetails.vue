  ...
  <div v-if="msg.ragTrace" class="message-meta">
    <details class="reasoning-details">
      ...
      <div class="reasoning-content">
        ...
        <div
          v-if="msg.ragTrace.evidence_relevance || msg.ragTrace.evidence_answerability"
          class="trace-line"
        >
          证据评分： 相关性 {{ msg.ragTrace.evidence_relevance || '—' }} / 可回答性
          {{ msg.ragTrace.evidence_answerability || '—' }}
          <span
            v-if="
              msg.ragTrace.evidence_confidence !== null &&
              msg.ragTrace.evidence_confidence !== undefined
            "
          >
        ...
        <div
          v-if="msg.ragTrace.evidence_ambiguity && msg.ragTrace.evidence_ambiguity !== 'none'"
          class="trace-line"
        >
        ...
        <div
          v-if="msg.ragTrace.hitl_options && msg.ragTrace.hitl_options.length"
          class="trace-line"
        >
          HITL选项：{{ msg.ragTrace.hitl_options.join(' / ') }}
        </div>
        <div v-if="msg.ragTrace.route" class="trace-line">评分决策：{{ msg.ragTrace.route }}</div>
        ...
        <div v-if="msg.ragTrace.retrieval_target_count !== undefined" class="trace-line">
          检索目标：{{ msg.ragTrace.retrieval_target_count }} 个 （必需
          {{ msg.ragTrace.retrieval_required_target_count ?? 0 }}，可选
        ...
        <div v-if="hasRetrievalFunnel(msg.ragTrace)" class="trace-line trace-funnel">
          检索漏斗：Milvus 召回 {{ msg.ragTrace.recall_count ?? '—' }} → 去重
          {{ msg.ragTrace.deduplicated_recall_count ?? msg.ragTrace.recall_count ?? '—' }} → 合并后
          {{ msg.ragTrace.post_merge_candidate_count ?? '—' }} → 精排输入
          {{ msg.ragTrace.rerank_candidate_count ?? msg.ragTrace.candidate_count ?? '—' }} → 输出
          top{{ msg.ragTrace.retrieval_top_k ?? (msg.ragTrace.retrieved_chunks || []).length }} （{{
            (msg.ragTrace.retrieved_chunks || []).length
          }}
          条）
        </div>
        ...
        <div
          v-if="
            msg.ragTrace.auto_merge_enabled !== null &&
            msg.ragTrace.auto_merge_enabled !== undefined
          "
          class="trace-line"
        >
        ...
        <div
          v-if="
            msg.ragTrace.auto_merge_applied !== null &&
            msg.ragTrace.auto_merge_applied !== undefined
          "
          class="trace-line"
        >
        ...
        <div
          v-if="msg.ragTrace.rerank_enabled !== null && msg.ragTrace.rerank_enabled !== undefined"
          class="trace-line"
        >
          Rerank已配置：{{ msg.ragTrace.rerank_enabled ? '是' : '否' }}
        </div>
        <div
          v-if="msg.ragTrace.rerank_applied !== null && msg.ragTrace.rerank_applied !== undefined"
          class="trace-line"
        >
        ...
        <div
          v-if="
            msg.ragTrace.rerank_candidate_count !== null &&
            msg.ragTrace.rerank_candidate_count !== undefined
          "
          class="trace-line"
        >
        ...
        <div
          v-if="msg.ragTrace.coverage_gap_codes && msg.ragTrace.coverage_gap_codes.length"
          class="trace-line"
        >
          覆盖缺口：{{ msg.ragTrace.coverage_gap_codes.join(' / ') }}
        </div>
        <div
          v-if="msg.ragTrace.coverage_gap_questions && msg.ragTrace.coverage_gap_questions.length"
          class="trace-line"
        >
        ...
        <div v-if="msg.ragTrace.complexity" class="trace-line trace-complexity">
          ...
          <span v-if="msg.ragTrace.complexity_reason" class="trace-detail"
            >（{{ msg.ragTrace.complexity_reason }}）</span
          >
        </div>

        <div
          v-if="msg.ragTrace.sub_questions && msg.ragTrace.sub_questions.length"
          class="trace-line"
        >
          <div class="trace-sub-questions">
            <div class="trace-sub-qs-header">
              子问题分解（{{ msg.ragTrace.sub_questions.length }} 个）：
            </div>
            <ul class="trace-sub-qs-list">
              <li
                v-for="(sq, sqIdx) in msg.ragTrace.sub_questions"
                :key="sqIdx"
                class="trace-sub-q-item"
              >
        ...
        <!-- 子 Agent 检索详情（可折叠） -->
        <div
          v-if="msg.ragTrace.sub_traces && msg.ragTrace.sub_traces.length"
          class="trace-sub-traces"
        >
          <details class="sub-traces-details">
            <summary class="sub-traces-title">
              <i class="fas fa-layer-group"></i> 子 Agent 检索详情（{{
                msg.ragTrace.sub_traces.length
              }}
              个）
            </summary>
            <div
              v-for="(st, stIdx) in msg.ragTrace.sub_traces"
              :key="stIdx"
              class="sub-trace-block"
            >
              <div class="sub-trace-header">
                子问题 {{ stIdx + 1 }}：{{ msg.ragTrace.sub_questions?.[stIdx] || st.query || '—' }}
              </div>
              <div v-if="st.retrieval_stage" class="trace-line">
                检索阶段：{{ st.retrieval_stage }}
              </div>
              <div v-if="st.retrieval_mode" class="trace-line">
                检索模式：{{ st.retrieval_mode }}
              </div>
              <div v-if="st.route" class="trace-line">评分决策：{{ st.route }}</div>
              <div
                v-if="st.retrieved_chunks && st.retrieved_chunks.length"
                class="sub-trace-chunks"
              >
        ...
        <div
          v-if="
            msg.ragTrace.initial_retrieved_chunks && msg.ragTrace.initial_retrieved_chunks.length
          "
          class="sources"
        >
          ...
          <ul class="sources-list">
            <li
              v-for="(chunk, sIndex) in msg.ragTrace.initial_retrieved_chunks"
              :key="sIndex"
              class="source-item"
            >
              ...
              <div class="source-meta-line">
                ...
                <span
                  v-if="chunk.rerank_score !== null && chunk.rerank_score !== undefined"
                  class="source-page"
                >
        ...
        <div
          v-if="
            msg.ragTrace.rewrite_retrieved_chunks && msg.ragTrace.rewrite_retrieved_chunks.length
          "
          class="sources"
        >
          ...
          <ul class="sources-list">
            <li
              v-for="(chunk, sIndex) in msg.ragTrace.rewrite_retrieved_chunks"
              :key="sIndex"
              class="source-item"
            >
              <div class="source-title-line">
...
const hasRetrievalFunnel = (trace: RagTrace) => {
  ...
  return (
    (trace.recall_count !== null && trace.recall_count !== undefined) ||
